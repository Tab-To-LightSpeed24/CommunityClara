    ...
    return (
      <div className="card backdrop-blur-sm bg-white/80 dark:bg-gray-900/80 border border-white/20 shadow-2xl">
        ...
        <div className="text-center py-8">
          ...
          <h4 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">No Analytics Data</h4>
          <p className="text-gray-600 dark:text-gray-300 mb-4">
            Analytics data will appear here once your server has more activity over time.
          </p>
          
          {/* Debug info in development */}
          {process.env.NODE_ENV === 'development' && (
            <div className="mt-4 p-3 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-left">
              <p className="text-sm text-yellow-800 dark:text-yellow-200 font-medium">Debug Info:</p>
              <p className="text-xs text-yellow-700 dark:text-yellow-300">
                Server: {serverId}<br/>
                Analytics Type: {typeof analytics}<br/>
                Is Array: {Array.isArray(analytics) ? 'Yes' : 'No'}<br/>
                Length: {analytics?.length || 'N/A'}<br/>
                Raw Data: {JSON.stringify(analytics)}
              </p>
            </div>
          )}
        </div>
