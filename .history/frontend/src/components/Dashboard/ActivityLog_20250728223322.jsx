...
const ActivityLog = ({ serverId }) => {
  ...
  useEffect(() => {
    const generateMockActivity = () => {
      const activityTypes = [
        { type: 'moderation', icon: '🛡️', color: 'text-primary-400', bg: 'bg-primary-500/10' },
        { type: 'violation', icon: '⚠️', color: 'text-warning-400', bg: 'bg-warning-500/10' },
        { type: 'user_joined', icon: '👋', color: 'text-success-400', bg: 'bg-success-500/10' },
        { type: 'message_deleted', icon: '🗑️', color: 'text-danger-400', bg: 'bg-danger-500/10' },
        { type: 'user_timeout', icon: '⏰', color: 'text-purple-400', bg: 'bg-purple-500/10' }
      ]
      ...
      const mockActivities = Array.from({ length: 15 }, (_, i) => {
        ...
        return {
          id: i + 1,
          ...activity,
          message: generateActivityMessage(activity.type),
          timestamp: new Date(Date.now() - Math.random() * 3600000 * 24).toISOString(),
          user: `User${Math.floor(Math.random() * 100) + 1}`
        }
      })

      setActivities(mockActivities.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)))
      setLoading(false)
    }

    const timer = setTimeout(generateMockActivity, 1000)
    return () => clearTimeout(timer)
  }, [serverId])

  const generateActivityMessage = (type) => {
    const messages = {
      moderation: ['Automatically moderated inappropriate content', 'Detected spam message', 'Applied content filters'],
      violation: ['Community guidelines violation detected', 'Toxic language flagged', 'NSFW content removed'],
      user_joined: ['New member welcomed to server', 'User verification completed', 'Role assignment successful'],
      message_deleted: ['Inappropriate message removed', 'Spam content deleted', 'Policy violation cleaned'],
      user_timeout: ['Temporary restriction applied', 'Cooling period initiated', 'User education triggered']
    }
    const typeMessages = messages[type] || ['Activity occurred']
