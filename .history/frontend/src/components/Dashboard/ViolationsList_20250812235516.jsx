...
const ViolationsList = ({ violations, serverId }) => {
  const uniqueViolations = React.useMemo(() => {
  ...
  const deduplicateViolations = (violations) => {
    ...
    // Create a Map to track unique violations by ID
    const uniqueMap = new Map()
    
    violations.forEach(violation => {
      const key = `${violation.id}-${violation.created_at}`
    ...
    console.log(`🔍 Deduplicated violations: ${violations.length} → ${deduplicated.length}`)
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-600 dark:text-gray-400 mb-2">
                      <span className="font-medium">
                        Confidence: {Math.min(Math.round(violation.confidence_score * 100), 100)}%
                      </span>
                      <span>#{details.channelName}</span>
                      <span className="font-bold text-purple-600">@{details.username}</span>
