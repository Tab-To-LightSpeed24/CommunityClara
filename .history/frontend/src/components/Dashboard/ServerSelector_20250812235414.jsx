...
const ServerSelector = ({ 
  ...
  const handleChange = (value) => {
    console.log('🔄 ServerSelector: Changing to server ID:', value)
  ...
  // Debug logging
  React.useEffect(() => {
    console.log('🎯 ServerSelector state:', {
      selectedServerId,
      serversCount: servers.length,
      selectedServerExists: servers.find(s => s.id === selectedServerId)
    })
  ...
  return (
    <div className="flex items-center space-x-3">
      ...
      <select
        id="server-select"
        value={selectedServerId || ''} // This should properly show the selected server
