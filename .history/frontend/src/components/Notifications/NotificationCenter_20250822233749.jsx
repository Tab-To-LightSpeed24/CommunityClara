...
const NotificationCenter = () => {
  ...
  // Fetch notifications from API with real-time polling
  const { data: notificationData, isLoading, error } = useQuery({
    queryKey: ['notifications'],
    queryFn: () => apiEndpoints.getUserNotifications(),
    select: (response) => {
      console.log('🔔 Frontend received notifications:', response.data)
  ...
  const markAsReadMutation = useMutation({
    mutationFn: (notificationId) => apiEndpoints.markNotificationRead(notificationId),
    onSuccess: () => {
  ...
  React.useEffect(() => {
    console.log('📊 Frontend notification states:')
    notifications.forEach(notif => {
      console.log(`  ${notif.id}: read=${notif.read}`)
    })
    console.log(`📈 Total: ${notifications.length}, Unread: ${unreadCount}`)
  }, [notifications, unreadCount])

  // Update the filter logic to include the new 'read' filter
  const filteredNotifications = notifications.filter(notif => {
    if (filter === 'all') return true
    if (filter === 'unread') return !notif.read
    if (filter === 'read') return notif.read // ADD THIS LINE
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg">
        {[
          ...
          { key: 'read', label: `Read (${notifications.length - unreadCount})`, icon: '✅' }, // ADD READ TAB
          ...
          filteredNotifications.map(notification => (
            <div
              ...
              className={`card border transition-all duration-200 hover:shadow-lg ${
                notification.read 
                  ? 'bg-gray-50/80 dark:bg-gray-800/60 border-gray-300 dark:border-gray-600 opacity-70' // READ STYLING
                  : 'bg-blue-50/80 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800 shadow-md' // UNREAD STYLING
              }`}
            >
              <div className="flex items-start space-x-4">
                ...
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                  notification.read 
                    ? 'bg-gray-200 dark:bg-gray-700' // READ ICON STYLING
                    : 'bg-blue-100 dark:bg-blue-900/50' // UNREAD ICON STYLING
                }`}>
                  <span className={`text-xl ${notification.read ? 'opacity-60' : ''}`}>
                    {notification.icon}
                  </span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2">
                    <h4 className={`font-medium ${
                      notification.read 
                        ? 'text-gray-500 dark:text-gray-400 line-through' // READ TITLE STYLING
                        : 'text-blue-900 dark:text-blue-100' // UNREAD TITLE STYLING
                    }`}>
                      {notification.title}
                    </h4>
                    {notification.severity && (
                      <span className={`text-xs font-medium ${
                        notification.read 
                          ? 'opacity-50 ' + getSeverityColor(notification.severity)
                  ...
                  <p className={`text-sm mt-1 ${
                    notification.read 
                      ? 'text-gray-400 dark:text-gray-500' // READ MESSAGE STYLING
                      : 'text-gray-600 dark:text-gray-400' // UNREAD MESSAGE STYLING
                  }`}>
                    {notification.message}
                  </p>
                  <div className={`flex items-center space-x-4 mt-2 text-xs ${
                    notification.read 
                      ? 'text-gray-400 dark:text-gray-500' // READ META STYLING
                      : 'text-gray-500 dark:text-gray-400' // UNREAD META STYLING
                  }`}>
                    {notification.server && (
