...
async def get_server_violations(
    ...
    """Get recent violations with REAL Discord data - ENHANCED"""
    try:
        # Verify server exists
        server = db.query(Server).filter_by(id=server_id).first()
        if not server:
        ...
        for violation, user in violations_query:
            # Use real channel name from violation record, or generate fallback
            if violation.channel_name:
                channel_name = violation.channel_name
            elif hasattr(violation, 'channel_name') and violation.channel_name:
                channel_name = violation.channel_name
            else:
                # Fallback for old violations without channel_name
                channel_name = f"#channel-{violation.channel_id[-2:]}" if violation.channel_id else "#unknown"
            
            # Use real message content if available
            message_content = violation.message_content if violation.message_content else "Content not stored for privacy"
            ...
            violations_data.append({
                ...
                "message_content": message_content  # NOW SHOWS REAL CONTENT
            })
