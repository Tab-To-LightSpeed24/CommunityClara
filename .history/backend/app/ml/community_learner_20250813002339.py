...
class CommunityLearner:
    ...
    async def get_server_insights(self, server_id: str) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as session:
                ...
                # Get recent violations - simplified query
    ...
    async def update_daily_analytics(self, server_id: str):
        """Update daily analytics for a server - FIXED"""
        try:
            with get_db_session() as session:
                server = session.query(Server).filter_by(id=server_id).first()
                ...
                # Calculate metrics
                violations_count = len(today_violations)
                false_positives_count = sum(1 for v in today_violations if v.false_positive)
                
                # Realistic health score calculation
                if violations_count > 0:
                    false_positive_rate = false_positives_count / violations_count
                    health_score = max(0.3, 0.95 - (violations_count * 0.02) - (false_positive_rate * 0.1))
                else:
                    health_score = 0.95
                ...
                if existing_analytics:
                    ...
                    existing_analytics.violations_detected = violations_count
                    existing_analytics.false_positives = false_positives_count
                    existing_analytics.community_health_score = health_score
                    existing_analytics.actions_taken = violations_count
                else:
                    # Create new analytics record
                    new_analytics = ServerAnalytics(
                        server_id=server_id,
                        date=datetime.utcnow(),
                        messages_processed=max(1, violations_count * 10),  # Estimate
                        violations_detected=violations_count,
                        false_positives=false_positives_count,
                        actions_taken=violations_count,
