...
class CommunityClara(commands.Bot):
    ...
    async def on_message(self, message: discord.Message):
        """Handle incoming messages with spam detection - FIXED"""
        # Ignore bot messages
        ...
        try:
            ...
            # 🚨 ADD SPAM DETECTION HERE - This was missing!
            ...
            logger.info(f"🔍 Spam analysis: score={spam_analysis['spam_score']}, is_spam={spam_analysis['is_spam']}")
    ...
    async def _handle_spam(self, message: discord.Message, spam_analysis: Dict):
        """Handle detected spam - FIXED METHOD"""
        try:
            ...
            try:
                embed = discord.Embed(
                    title="🚫 Spam Detected",
                    description=f"Your messages in {message.guild.name} were detected as spam.",
                    color=0xFF6600
                )
                
                embed.add_field(
                    name="Detected Patterns",
                    value=f"• {chr(10).join(spam_analysis['reasons'])}",
                    inline=False
                )
                
                embed.add_field(
                    name="Please",
                    value="• Slow down your messaging\n• Avoid repeating the same content\n• Write meaningful messages",
                    inline=False
                )
                ...
                logger.warning(f"Could not send spam DM: {dm_error}")
