...
class CommunityClara(commands.Bot):
    ...
    async def on_message(self, message: discord.Message):
        ...
        if message.content.lower() == '!clara testspam':
    ...
    async def _get_server_config(self, guild_id: int) -> Optional[Dict]:
        ...
        try:
            with get_db_session() as session:
                ...
                logger.info(f"🔧 FRESH config from DB: toxicity={server.toxicity_threshold}, nsfw={server.nsfw_threshold}")
    ...
    # Add this method to your CommunityClara class in discord_bot_minimal.py
    async def _reset_user_warnings(self, user_id: int, server_id: int):
        """Reset user's warning count after 3rd strike"""
        try:
            with get_db_session() as session:
                # Update all warn1 and warn2 to resolved status
                session.query(Violation).filter(
                    Violation.user_id == str(user_id),
                    Violation.server_id == str(server_id),
                    Violation.action_taken.in_(['warn1', 'warn2'])
                ).update({Violation.action_taken: 'resolved'})
                session.commit()
