...
class Server(Base):
    ...
    nsfw_auto_delete = Column(Boolean, default=True)
    nsfw_auto_timeout = Column(Boolean, default=False)
    nsfw_auto_kick = Column(Boolean, default=False)
    nsfw_auto_ban = Column(Boolean, default=False)
...
class User(Base):
    """Enhanced User model with Google OAuth support - NO UserServer relationship"""
    __tablename__ = "users"
    ...
    # Relationships - ONLY violations, NO user_servers
...
class Violation(Base):
    ...
    log_message_id = Column(String, nullable=True) # ID of the alert message for reaction tracking
