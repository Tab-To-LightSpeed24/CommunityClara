...
# Add these routes to backend/app/api/routes.py
...
async def get_current_user(token: str = Depends(security), db: Session = Depends(get_db)) -> User:
    ...
    try:
        payload = auth_service.verify_jwt_token(token.credentials)
        if not payload:
...
async def google_auth(request: dict, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Verify Google token
        google_info = await auth_service.verify_google_token(google_token)
        if not google_info:
            raise HTTPException(status_code=400, detail="Invalid Google token")
        
        # Create or update user
        user = await auth_service.create_or_update_user(google_info, db)
...
async def get_current_user_info(current_user: User = Depends(get_current_user)):
    ...
    try:
        # Handle last_login field properly - it's stored as string in database
        last_login_iso = None
        if current_user.last_login:
            if isinstance(current_user.last_login, str):
                # It's already a string (ISO format)
                last_login_iso = current_user.last_login
            else:
                # It's a datetime object
                last_login_iso = current_user.last_login.isoformat()
        ...
        return {
            "user": {
                ...
                "last_login": last_login_iso  # Fixed - no .isoformat() call on string
            }
        }
    except Exception as e:
...
async def update_profile(
    ...
    try:
        ...
        allowed_fields = ['username', 'display_name', 'theme', 'timezone', 'language', 'notification_preferences']
...
async def get_server_stats(server_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return ServerStatsResponse(
            ...
            nsfw_auto_kick=getattr(server, 'nsfw_auto_kick', False)  # ADD THIS
        )
...
async def update_server_config(
    ...
    try:
        ...
        if config.nsfw_auto_kick is not None:
            old_val = getattr(server, 'nsfw_auto_kick', False)
            server.nsfw_auto_kick = bool(config.nsfw_auto_kick)
            changes.append(f"nsfw_auto_kick: {old_val} → {server.nsfw_auto_kick}")
...
async def update_server_settings(
    ...
    try:
        ...
        # NSFW Settings Updates
        if settings.nsfw_allowed is not None:
            old_value = getattr(server, 'nsfw_allowed', False)
            server.nsfw_allowed = settings.nsfw_allowed
            changes.append(f"nsfw_allowed: {old_value} → {server.nsfw_allowed}")
            
        if settings.nsfw_auto_delete is not None:
            old_value = getattr(server, 'nsfw_auto_delete', True)
            server.nsfw_auto_delete = settings.nsfw_auto_delete
            changes.append(f"nsfw_auto_delete: {old_value} → {server.nsfw_auto_delete}")
            
            server.nsfw_auto_timeout = settings.nsfw_auto_timeout
            changes.append(f"nsfw_auto_timeout: {old_value} → {server.nsfw_auto_timeout}")

        if settings.nsfw_auto_kick is not None:
            old_value = getattr(server, 'nsfw_auto_kick', False)
            server.nsfw_auto_kick = settings.nsfw_auto_kick
            changes.append(f"nsfw_auto_kick: {old_value} → {server.nsfw_auto_kick}")

        if settings.nsfw_auto_ban is not None:
            old_value = getattr(server, 'nsfw_auto_ban', False)
            server.nsfw_auto_ban = settings.nsfw_auto_ban
            changes.append(f"nsfw_auto_ban: {old_value} → {server.nsfw_auto_ban}")
...
async def get_user_notifications(
    ...
    """Get user notifications with read state - UPDATED"""
    try:
        ...
        # Create notifications from violations with read state
        for violation in recent_violations:
            server = db.query(Server).filter_by(id=violation.server_id).first()
            server_name = server.name if server else "Unknown Server"
            notification_id = f"violation_{violation.id}"
            
            # Check if this notification is marked as read
            is_read = notification_read_state.get(notification_id, False)
        ...
        # Add system notifications with read state
        system_notifications = [
            {
                ...
                "read": notification_read_state.get("system_update_1", False),
                "icon": "🔄",
                "severity": "low"
            },
            {
                ...
                "timestamp": (datetime.utcnow() - timedelta(hours=5)).isoformat(),
                "read": notification_read_state.get("daily_report_1", False),
                "icon": "📊",
                "severity": "medium"
            }
        ]
        ...
        logger.info(f"📊 Read state summary: {sum(1 for n in notifications if n['read'])} read, {unread_count} unread")
...
notification_read_state = {}  # In production, use database
...
async def mark_notification_read(
    ...
    try:
        ...
        # Store read state (in production, save to database)
...
async def mark_all_notifications_read(
    ...
    try:
        ...
        # Mark all violation notifications as read
        for violation in recent_violations:
            notification_id = f"violation_{violation.id}"
            notification_read_state[notification_id] = True
            logger.info(f"  ✓ Marked {notification_id} as read")
        
        # Mark system notifications as read
        notification_read_state["system_update_1"] = True
        notification_read_state["daily_report_1"] = True
        
        total_marked = len(recent_violations) + 2
        
        logger.info(f"✅ Successfully marked {total_marked} notifications as read")
        
        return {
            "success": True,
            "message": f"All {total_marked} notifications marked as read",
            "user_id": user_id,
            "updated_count": total_marked,
            "timestamp": datetime.utcnow().isoformat()
        }
...
async def get_unread_notification_count(
    ...
    """Get unread notification count for badges/indicators"""
    try:
        # Get recent violations count as proxy for unread notifications
        recent_violations = db.query(Violation).filter(
            Violation.created_at >= datetime.utcnow() - timedelta(days=7)
        ).count()
        
        # Add system notifications
        unread_count = min(recent_violations + 2, 10)  # Cap at 10 for UI
